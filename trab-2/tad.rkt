...
(define (explorar sala jogador)
  (cond
    ...
    [else
     ...
     (for-each displayln (map objeto-nome (ambiente-objetos sala)))
     ...
     (define jog-atual ((objeto-interacao obj) jogador sala))
     ; com o enigma resolvido, remove-se o objeto do ambiente
     (define ambiente-atual (struct-copy ambiente sala
                                       [objetos (remove obj (ambiente-objetos sala))]))
...
(define sala-controle (ambiente "Sala de Controle" "\nO ponto de partida, onde você fará a análises e monitoramentos do sistema."
                                ...
                                data-center ; saídas disponíveis
                                ))
